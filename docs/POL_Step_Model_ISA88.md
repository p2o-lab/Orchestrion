...
> (now §4a). All corrected below; the evidence ledger is in
> [`progress/010`](progress/010_m5_step_model_correction.md) §11.

Every claim is tagged. This exists because every defect in this project came from the same habit —
reasoning forward from a prior decision instead of returning to the source.
...
| **`[DERIVED]`** | inference *from* cited clauses. **Every defect so far has lived here.** |
...
> false** — a paraphrase wearing a clause number, which is the exact failure `010` §4 exists to catch.
