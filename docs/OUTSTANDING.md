...
builder; both were then found structurally wrong against IEC 60848 and ISA-88, and corrected in
full - 12 units, two audit passes, the core defect proven dead over HTTP.

What remains is **M5.5 → M5.8**, plus the corrections deliberately left behind at the time.
...
endpoint. `GET …/runs/{id}` is *truthful mid-run* - it was not before the P1 fix of the 2026-08-07
backend audit, which would have made this view render an empty chart.

Four separable pieces, in rough size order:

- ~~**a. Run status hook + launch/abort wiring**~~ - **BUILT 2026-09-06.** `hooks/useRun.ts`
...
- ~~**e. A helper to launch N VirtualPEA instances as a demo plant**~~ - **BUILT 2026-09-06.**
 `virtual_pea/plant.py`: `--count N` serves N modules from one process, generating a manifest per port (the endpoint lives
...
> ISA-88. The view must render `state/classification.py`, which was built for exactly this, and
...
The *engine* half is done: held/paused reported, a disconnect fails the run naming the PEA
...
**B1–B6 and B9 come from the M5.4 correction; B7 and B8 surfaced on the 2026-08-08 end-to-end doc
read** - B7 from a conflict between two authority docs, B8 from the VirtualPEA's parked list.
**None blocks M5.5.**
...
the runtime check won - is recorded with the intended shape. Worth doing **around M5.5b**:
...
lets it through. Now reachable from the UI, since the condition editor made compounds authorable.
...
where this must be fixed.** Small change to one function.
...
of the clear disconnect message the step model calls for. A worse diagnosis, not a worse outcome.
...
*From the VirtualPEA's parked list; not tracked anywhere else until 2026-08-08.*
...
cause. Parked deliberately on 2026-07-17 on the grounds that it is *"a crash, not a
...
- **`Not` in the condition union**. Its absence means `[IEC 60848:2013]`
...
they were recorded when the control handshake closed out, but not carried
...
### Parked at M1, still parked

*Also missed by the first version of this file.* Each was parked **with its clause**, so none needs
re-deriving - `parser.py` carries the citations at the sites concerned.
...
- **§12 runtime type/version verification** - the **MTP half is on `Pea`** (`manufacturer_uri`,
  ...
  And the parser work already proved **§12.3 cannot pass against HC30**, whose `DeviceRevision` is
...
- **Plant topology (ports / connections)** - deferred at M2 with a seam already in place: the schema
  hangs everything off `project_id`/`pea_id`, so a `Connection`/`Port` table attaches without
  disturbing projects or PEAs. Recipes were the other half of
...
wanting. The 2026-08-07 backend audit was deliberately scoped to the recipe path.
...
B4 is precisely the kind of defect that only appears there.

### D2 · Modules not re-read in the backend audit

`mtp/` (~1,750 lines) · `virtual_pea/` · `events.py` · `db/` · the non-recipe API modules. M1/M2 code,
unchanged since July, covered by their own suites and audited when they were written. Not suspected -
...
and `react-refresh/only-export-components`. **They predate the recipe work** (verified by linting a
stashed tree), so no unit added to them. Not addressed either. Worth a small dedicated pass, mostly so the
...
one still claimed IEC 60848 was not in `../standards/`. Only the later, read-in-full passes turned up
the invented **OR rail** in an
ASCII diagram that the earlier ones had scrolled past. If you need to know whether a doc is
