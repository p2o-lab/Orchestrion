...
> in ISA-88 / IEC 61512). M5.0–M5.4 were built first, then corrected: the step model and the chart
> were rebuilt against IEC 60848 + ISA-88, **complete (2026-08-08)**.
> **Next: M5.5, the live execution view.** Faceplates (Blatt 2) and alarms
> (Blatt 6/7) remain later v2 items. The real-PEA end-to-end demo (M4 Step 6) is still owed.
> **What is left overall: [`OUTSTANDING.md`](OUTSTANDING.md).**
...
- **Test artifacts** *(re-assessed 2026-07-16 by measuring each file's declared MTP version)*:
...
built except the event log. **Next: M4 - event log + polish.**
This section is only the original plan of record.

*(Historical trail: M0 scaffold; M1 parser, built against **HC30**
`../reference/Recipol/Recipol/artifacts/2026-05-18-HC30_Stirring_V8.aml` behind a version-detection seam (§5),
normative structure from primary sources; M2 VirtualPEA + POL backend + frontend;
M3 control; live values.)*

> **Endpoint path - `/api/health`, not `/health`.** The Vite dev proxy forwards `/api/*` **without a `rewrite`**, so the backend must own the `/api` prefix; a bare `/health` is shadowed by the `:5173` dev server and never reaches uvicorn. *(Decided 2026-07-16 - see open item 5.)*

> **This section is the original plan of record, not a status report.** As of 2026-07-16 `pyproject.toml` and the `/api/health` endpoint are **done**.
