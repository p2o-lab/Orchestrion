...
("boxes = steps, arrows = transitions" - **wrong**, see §1) and the earlier AND/OR bar-node model.

> **IMPLEMENTED (2026-08-08).** The builder now follows this document: two node kinds, the receptivity on
...
Both are licensed documents held outside this repository, alongside `../standards/`. **Neither may
...
> symbol and upgraded four others to `[CITED]` - the detail is in the box below.
...
> **They are no longer *unavailable*, only *unread*:** the text is in hand and any
> of them can be upgraded by opening the clause. None is load-bearing for what is built - but
> **a tag is never upgraded without reading the source**, so they stay `[TITLE-ONLY]` until someone does.
...
> and what the first build's AND/OR bar nodes got wrong. Both modelled a drawing convention as a graph
...
> the 2026-08 correction and describe the engine **as it then was** - so phrases like *"currently makes the author
...
> the two gates, the `Always` rule over the whole `from_ids`, and deliberate OR
> arbitration. They are kept because *why* the design is what it is only makes sense against
> what it replaced. **For what the code does now, read the engine.**
...
*(An earlier draft justified selection with "Figure 25 - selection of procedural elements". **That is a
...
> `barSpans` now returns entries for transitions only. The rail had been drawn before the symbol
> tables were obtained, and a note recording that risk was not enough to prevent it shipping.
...
- **Record it as SFC arbitration, not GRAFCET conformance.** GRAFCET's own answer is that exclusivity
...
4. **The condition editor cannot author `And`/`Or` at all** *(added 2026-08-05, read directly)*.
   ...
   two-clause one cannot be typed. **FIXED** (2026-08-08) - the editor now edits
