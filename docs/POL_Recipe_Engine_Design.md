...
>
> **Status:** design locked 2026-07-26. **M5.0–M5.4 built, then corrected in full against
> IEC 60848 + ISA-88; that correction is complete (2026-08-08).
> Next is M5.5, the live execution view.** What is left:
...
> success.**
>
> The **§6 roadmap below is also superseded** for M5.4 onward - the corrected build order replaces
> it, and absorbs part of M5.5 (the run endpoint). M5.5 keeps the live execution *view*; M5.6–M5.8
> stand.
...
code and docs as *design-not-dictated* - never dressed up as a clause. (Rule 1 still binds the
...
- `engine.py` - `RecipeEngine`: one async task per run; walks the step/transition graph; tracks run
  ...
  > The correction also added `driver.py` (the `StepDriver` seam) and `runs.py` (`RunManager`), which
...
- Example (linear then parallel), the JSON that drives both the chart and the engine.
 > **Rewritten 2026-08-08 - the original taught an anti-pattern since removed.** Both transitions
...
## 6. Incremental roadmap (each increment tested before the next is started)
...
- Verify the ISA-88 structure and state model against IEC 61512-1 before modelling; cite every
 borrowed term, so the implementation can be followed back to the standard by anyone holding it.
 Explicitly mark the MTP↔recipe binding and the condition scheme as *our design* - the
 orchestration layer is not yet standardized. Never guess a clause.
- One increment at a time, tested and recorded before the next is started.
